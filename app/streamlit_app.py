...
def main_app():
    ...
    if show_perf:
        ...
        m1.metric("Accuracy", "94.5%")
        m2.metric("Precision", "90.53%")
        m3.metric("Recall", "99.40%")
        m4.metric("F1 Score", "94.76%")

        st.subheader("Confusion Matrix (Test Set)")
        cm_data = np.array([[448, 52], [3, 497]]) 
        ...
        if uploaded_file:
            ...
            if face_img is not None:
                ...
                with st.expander("📝 Technical Data"):
                    st.write(f"**Integrity Status:** {'🚩 Compromised' if raw_score > 0.65 else '🟢 Intact'}")
